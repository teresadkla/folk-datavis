...
const GraficoTemasPorRegiao = ({ active }) => {
  ...
  const handleDropdownToggle = (type) => {
    if (type === 'viewMode') {
      // Se estamos abrindo o viewMode, fechamos os outros dropdowns
      setIsDropdownOpen(false);
      setIsDropdownCategoriaOpen(false);
      setShowViewModeDropdown(!showViewModeDropdown);
    } else if (type === 'filter') {
      // Se estamos abrindo o filter, fechamos os outros dropdowns
      setShowViewModeDropdown(false);
      setIsDropdownCategoriaOpen(false);
      setIsDropdownOpen(!isDropdownOpen);
    } else if (type === 'categoria') {
      // Se estamos abrindo o categoria, fechamos os outros dropdowns
      setShowViewModeDropdown(false);
      setIsDropdownOpen(false);
  ...
  const handleViewModeChange = (mode) => {
    if (modoVisualizacao !== mode) {
      setModoVisualizacao(mode);
    }
    setShowViewModeDropdown(false);
  };
    ...
    <div className="dotplotPT-container">
      ...
      <div className="dotplotPT-filters">
        <div className="dropdown-container">
          <button
            className="dropdown-btn-PT view-mode-btn-PT"
            onClick={() => handleDropdownToggle('viewMode')}
          >
            Visualizar por <span className="chevron">▼</span>
          </button>
          {showViewModeDropdown && (
            <div className="dropdown-content-PT">
              <label className="checkbox-item-PT">
                <input
                  type="checkbox"
                  checked={modoVisualizacao === 'temas'}
                  onChange={() => handleViewModeChange('temas')}
                />
                Temas
              </label>
              <label className="checkbox-item-PT">
                <input
                  type="checkbox"
                  checked={modoVisualizacao === 'categorias'}
                  onChange={() => handleViewModeChange('categorias')}
                />
                Categorias
              </label>
              <label className="checkbox-item-PT">
                <input
                  type="checkbox"
                  checked={modoVisualizacao === 'instrumentos'}
                  onChange={() => handleViewModeChange('instrumentos')}
                />
                Instrumentos
              </label>
            </div>
