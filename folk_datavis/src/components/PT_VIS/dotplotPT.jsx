...
const getFlowerType = (count, quintiles) => {
  if (count <= quintiles[0]) return 'flower1';
  if (count <= quintiles[1]) return 'flower2';
  if (count <= quintiles[2]) return 'flower3';
  if (count <= quintiles[3]) return 'flower4';
  return 'flower5';
};

const getFlowerPath = (type) => {
  const paths = {
    flower1: "M15.93,18.24C14.03,7.5,23.46.65,23.46.65c9.41,9.14,6.23,19.31,6.23,19.31,14.39-3.95,14.58,7.95,14.58,7.95-13.8-5.94-13.24,14.64-13.24,14.64.86,19.06-8.52,17.24-8.52,17.24-7.11.66-7.94-17.01-7.94-17.01C13.39,21.76,1.06,24.51,1.06,24.51c6.83-10.94,14.88-6.27,14.88-6.27h-.01ZM56.9,28.97c-7.16,6.12-15.64,17.16-20.64,26.03-3.88,6.9-4.69,7.88-1.27,5.41,8.56-5.93,18.45-13.86,25.62-22.03,5.57-6.23,7.31-10.02,6.75-12.17-1.77-4.29-7.8.5-10.28,2.61l-.19.16h0ZM33.98,68.91c4.84,0,8.77,3.93,8.77,8.77s-3.93,8.77-8.77,8.77-8.77-3.93-8.77-8.77,3.93-8.77,8.77-8.77Z", // path atual - para menores ocorrências
    flower2: "M57.04,132.66c2.4,10.64-6.7,17.92-6.7,17.92-9.83-8.69-7.12-19-7.12-19-14.19,4.61-14.93-7.27-14.93-7.27,14.06,5.3,12.55-15.24,12.55-15.24-1.74-19,7.71-17.62,7.71-17.62,7.08-.99,8.73,16.63,8.73,16.63,2.16,20.94,14.34,17.63,14.34,17.63-6.32,11.24-14.57,6.95-14.57,6.95h-.01ZM15.61,123.84c6.87-6.44,14.82-17.87,19.41-26.96,3.55-7.07,4.32-8.09,1.02-5.46-8.27,6.32-17.79,14.7-24.57,23.2-5.28,6.49-6.83,10.35-6.18,12.47,1.97,4.2,7.77-.87,10.15-3.08l.18-.17h0ZM15.59,18.57C13.2,7.93,22.3.65,22.3.65c9.83,8.69,7.12,19,7.12,19,14.19-4.61,14.93,7.27,14.93,7.27-14.06-5.3-12.55,15.24-12.55,15.24,1.74,19-7.71,17.62-7.71,17.62-7.08.99-8.73-16.63-8.73-16.63C13.2,22.21,1.02,25.52,1.02,25.52c6.32-11.24,14.57-6.95,14.57-6.95ZM57.02,27.39c-6.87,6.44-14.82,17.87-19.41,26.96-3.55,7.07-4.32,8.09-1.02,5.46,8.27-6.32,17.79-14.7,24.57-23.2,5.28-6.49,6.83-10.35,6.18-12.47-1.97-4.2-7.77.87-10.15,3.08l-.18.17h0ZM36.32,66.85c4.84,0,8.77,3.93,8.77,8.77s-3.93,8.77-8.77,8.77-8.77-3.93-8.77-8.77,3.93-8.77,8.77-8.77Z", // adicionar path da flower2
...
const GraficoTemasPorRegiao = ({ active }) => {
  ...
  useEffect(() => {
    ...
    // Calcular quintis para determinar o tipo de flor
    const counts = dadosVisualizacao.map(d => d.count).sort(d3.ascending);
    const quintiles = [
      d3.quantile(counts, 0.2),
      d3.quantile(counts, 0.4),
      d3.quantile(counts, 0.6),
      d3.quantile(counts, 0.8)
    ];

    const rScale = d3.scaleSqrt().domain([1, d3.max(dadosVisualizacao, d => d.count)]).range([40, 100]);
    ...
    const visiveis = dadosVisualizacao.filter(
      ...
      ...d,
      flowerType: getFlowerType(d.count, quintiles)
    }));
